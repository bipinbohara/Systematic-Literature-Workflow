...
# Adds two columns if missing: Abstract (pass-through) and llm_output
...
def _build_user_prompt(title: str, abstract: str) -> str:
    parts = [USER_PROMPT_PREFIX, f"TITLE: {json.dumps(title)}"]
    if abstract:
        parts.append(f"\nABSTRACT: {json.dumps(abstract)}")
    return "".join(parts)

def _call_completions(title: str, abstract: str) -> str:
    prompt = f"System: {SYSTEM_PROMPT}\n\nUser:\n{_build_user_prompt(title, abstract)}"
    payload = {
        "model": LLM_MODEL,
        "prompt": prompt,
        # "max_tokens": 16,  # keep short if you want to enforce terseness
        "temperature": 0.0,
        "stream": False,
    }
...
def _call_chat(title: str, abstract: str, url: str) -> str:
    payload = {
        "model": LLM_MODEL,
        "messages": [
...
def call_llm(title: str, abstract: str) -> str:
    ...
    try:
        if is_chat:
            return _call_chat(title, abstract, url)
        else:
            return _call_completions(title, abstract)
    except RuntimeError as e:
        msg = str(e)
        if (not is_chat) and "LLM HTTP 404" in msg:
            chat_url = url.rsplit("/v1/completions", 1)[0] + "/v1/chat/completions"
            try:
                return _call_chat(title, abstract, chat_url)
            except Exception:
...
def main() -> None:
    ...
    with INPUT_CSV.open("r", newline="", encoding="utf-8-sig") as f_in, \
        ...
        # Ensure Abstract and llm_output appear in output
        out_fields = list(headers)
        if ABSTRACT_COL not in out_fields:
            out_fields.append(ABSTRACT_COL)
        ...
        for row in reader:
            ...
            # Keep Abstract column even if input lacked it
