...
def download_full_text_pdf(pmid):
    """
    Attempts to download the full-text PDF for a given PubMed article using its PMID.
    Saves it in PDF_DIR as 'PMID_{pmid}.pdf' if available.
    """
    logging.info(f"Attempting to access full text for PMID: {pmid}")
    
    # Step 1: Fetch XML metadata to find PMC ID
    efetch_url = f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id={pmid}&retmode=xml"
    headers = {
    ...
    response = requests.get(efetch_url, headers=headers)
    if response.status_code != 200:
        logging.warning(f"Failed to fetch article metadata (status {response.status_code}) for PMID {pmid}")
        return f"Error: Unable to fetch article metadata for PMID {pmid}"

    root = ET.fromstring(response.content)
    pmc_id_tag = root.find(".//ArticleId[@IdType='pmc']")
    if pmc_id_tag is None:
        logging.info(f"No PMC ID found for PMID: {pmid}")
        return f"No PMC ID found for PMID: {pmid}"

    pmc_id = pmc_id_tag.text

    # Step 2: Check if open access article
    pmc_url = f"https://www.ncbi.nlm.nih.gov/pmc/articles/{pmc_id}/"
    pmc_response = requests.get(pmc_url, headers=headers)
    if pmc_response.status_code != 200 or "This article is available under a" not in pmc_response.text:
        logging.info(f"Article is not fully open access or PMC page inaccessible: {pmc_url}")
        return f"Not open access or inaccessible: {pmc_url}"

    # Step 3: Attempt to download PDF
    pdf_url = f"https://www.ncbi.nlm.nih.gov/pmc/articles/{pmc_id}/pdf"
    pdf_response = requests.get(pdf_url, headers=headers)
    if pdf_response.status_code != 200:
        logging.warning(f"Failed to download PDF for PMID {pmid} (status {pdf_response.status_code})")
        return f"Error downloading PDF for PMID {pmid}"

    # Step 4: Save PDF to target path
    ...
    return f"Downloaded and saved PDF: {pdf_path}"
